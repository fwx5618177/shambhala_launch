...
const PortfolioSection: FC<PortfolioSectionProps> = ({ type }) => {
  ...
  const fetchTransacData = async () => {
    try {
      const response = await api.get("/api/list");
      let arr: {
        protocol: { src: string; alt: string; label: string; size: number };
        network: { src: string; alt: string; label: string; size: number };
        amount: { fusdt: number; usdt: number };
        investedProducts: string;
        assets: any;
        date: number;
      }[] = [];
      //   response.data.forEach((tag: { mount: any }) => {
      arr.push({
        protocol: {
          src: "/fluid.svg",
          alt: "Fluid",
