...
const TabSwitcher: FC<{ activeTab: string; onTabSwitch: (tab: 'defi' | 'transactions') => void }> = ({ activeTab, onTabSwitch }) => (
  <div className="w-full flex justify-start space-x-[260px] text-[24px] font-600 border-b border-[#EBEBEB]">
    {['defi', 'transactions'].map((tab) => (
      <div
        key={tab}
        className={`px-2 py-2 text-center cursor-pointer ${activeTab === tab ? 'border-b border-primary' : 'bg-white'}`}
...
const PortfolioSection: FC<PortfolioSectionProps> = ({ type }) => {
  const [activeTab, setActiveTab] = useState<'defi' | 'transactions' | 'detail'>(type);
  ...
  return (
    <section className="bg-bg-primary w-full h-screen py-[135px] px-[105px]">
      {(type === 'defi' || type === 'transactions') && (
        <>
          <h1 className="text-lg font-800 text-primary mb-[85px]">My portfolio</h1>
      ...
      {type === 'detail' && (
        <>
          <div className="text-lg font-800 text-primary mb-[85px] flex items-center gap-[5px] capitalize">
            <Image src={`/${name}.svg`} alt={name as string} width={50} height={50} />
          ...
          <div className="mt-[46px] flex flex-col text-primary">
            ...
            <div className="w-full flex items-center justify-between">
              ...
              <p className="text-[14px] font-500 text-[#929292] flex items-center">
                Data updated <span className="text-primary ml-1">1 minute</span> ago
                <Image src="/scroll.svg" width={24} height={24} alt="Scroll" className="cursor-pointer ml-2" onClick={handleUpdate} />
              </p>
            </div>
          </div>
      ...
      <div className="my-[50px]">
        <Table columns={columns as Column<RowObject>[]} dataSource={dataSource} type={activeTab === 'transactions' ? 'card' : 'normal'} />
      </div>
    </section>
  );
};
